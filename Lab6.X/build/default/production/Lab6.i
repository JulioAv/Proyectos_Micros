...
main:
   ...
   BANKSEL OPTION_REG
   bsf OPTION_REG, 0
   bsf ((OPTION_REG) and 07Fh), 6
   bcf ((OPTION_REG) and 07Fh), 5 ; TMR0 con 16 de prescaler
...
loop:
    BANKSEL INTCON
    btfsc ((INTCON) and 07Fh), 2 ; Se verifica en ambos casos que se cumpla el
    call disp1 ; overflow del tmr0 para pasar el valor al PORTC
    BANKSEL INTCON ; y hacer el multiplexado
    btfsc ((INTCON) and 07Fh), 2
    call disp2
    BANKSEL PIR1
    btfsc ((PIR1) and 07Fh), 0 ; Se verifica el overflow del tmr1 para incrementar
